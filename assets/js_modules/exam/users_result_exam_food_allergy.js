...
var UsersResultExamFoodAllergy = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('exam/users_result_exam_food_allergy/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('exam/users_result_exam_food_allergy/save');
	...
	deleteRecord: function(){
		var frm_action = site_url('exam/users_result_exam_food_allergy/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url('exam/users_result_exam_food_allergy/index/'+ this.current_page));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		UsersResultExamFoodAllergy.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return UsersResultExamFoodAllergy.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		UsersResultExamFoodAllergy.current_page = Math.abs(param_current_page);
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		UsersResultExamFoodAllergy.confirmDelete(pExamId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		UsersResultExamFoodAllergy.deleteRecord();
	});
