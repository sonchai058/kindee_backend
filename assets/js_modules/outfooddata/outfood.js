...
var Outfood = {
	...
	loadPreview: function (id) {
		$.ajax({
			method: "GET",
			url: site_url("outfooddata/outfood/preview/" + id),
	...
	saveFormData: function () {
		var frm_action = site_url("outfooddata/outfood/save");
	...
	saveEditForm: function () {
		...
		var frm_action = site_url("outfooddata/outfood/update");
	...
	confirmDelete: function (pOutfoodId, irow) {
		$('[name="encrypt_outfood_id"]').val(pOutfoodId);
	...
	deleteRecord: function () {
		var frm_action = site_url("outfooddata/outfood/del");
				...
				if (results.is_successful) {
					...
					setTimeout(function () {
						$(window.location).attr(
							"href",
							site_url("outfooddata/outfood/index/" + this.current_page)
						);
					}, 500);
				} else {
					alert_type = "danger";
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Outfood.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Outfood.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Outfood.current_page = Math.abs(param_current_page);
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pOutfoodId = $(this).attr('data-outfood_id');

		Outfood.confirmDelete(pOutfoodId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		Outfood.deleteRecord();
	});
	setDropdownList('#user_delete');
	setDropdownList('#user_add');
	setDropdownList('#user_update');
	setDropdownList('#fag_allow');
	setDropdownList("#rmat_id");
...
$(document).ready(function() {
    ...
    $(document).on('click', '.image-cancel', function() {
		...
		$.ajax({
			method: 'POST',
			url: site_url('outfooddata/outfood/deleteBlogImage'),
			dataType: 'json',
			data:fdata,
			success: function (results) {
				console.log(results);
    ...
    if (window.File && window.FileList && window.FileReader) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            picReader.addEventListener('load', function (event) {
				...
				//console.log(fdata);
				fdata += '&outfood_id='+data_id;
				$.ajax({
					method: 'POST',
					url: site_url('outfooddata/outfood/uploadfile'),
					dataType: 'json',
					data : fdata,
					success: function (results) {
						console.log(results);
