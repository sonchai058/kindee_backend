...
var Package = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('packagedata/package/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('packagedata/package/save');
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						alert_type = 'success';
						setTimeout(function(){
							$(window.location).attr('href', site_url('dashboard_user'));
						}, 500);
					}else{
						alert_type = 'danger';
					}
					...
					if(results.is_successful){
						...
						if(add_array.length) {
							var fdata = 'data='+JSON.stringify(add_array)+'&package_id='+results.id;
							fdata += '&' + csrf_token_name + '=' + $.cookie(csrf_cookie_name);
							$.ajax({
								method: 'POST',
								url: site_url('packagedata/package/setPackageImages'),
								dataType: 'json',
								data:fdata,
								success: function (results) {
									console.log(results);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('packagedata/package/update');
	...
	confirmDelete: function (pPackageId,  irow){
		$('[name="encrypt_package_id"]').val(pPackageId);
	...
	deleteRecord: function(){
		var frm_action = site_url('packagedata/package/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url('packagedata/package/index/'+ this.current_page));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Package.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Package.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Package.current_page = Math.abs(param_current_page);
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pPackageId = $(this).attr('data-package_id');

		Package.confirmDelete(pPackageId,  row_num);
	});//click
...
$(document).ready(function() {
    ...
    $(document).on('click', '.image-cancel', function() {
		...
		$.ajax({
			method: 'POST',
			url: site_url('packagedata/package/deletePackageImage'),
			dataType: 'json',
			data:fdata,
			success: function (results) {
				console.log(results);
    ...
    if (window.File && window.FileList && window.FileReader) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            picReader.addEventListener('load', function (event) {
				...
				//console.log(fdata);
				fdata += '&package_id='+data_id;
				$.ajax({
					method: 'POST',
					url: site_url('packagedata/package/uploadfile'),
					dataType: 'json',
					data : fdata,
					success: function (results) {
						console.log(results);
