...
var Pro = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('prodata/pro/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('prodata/pro/save');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('prodata/pro/update');
	...
	confirmDelete: function (pProId,  irow){
		$('[name="encrypt_pro_id"]').val(pProId);
	...
	deleteRecord: function(){
		var frm_action = site_url('prodata/pro/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url('prodata/pro/index/'+ this.current_page));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	if(typeof param_current_page != 'undefined'){
		Pro.current_page = Math.abs(param_current_page);
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pProId = $(this).attr('data-pro_id');

		Pro.confirmDelete(pProId,  row_num);
	});//click
...
$(document).ready(function() {
    ...
    $(document).on('click', '.image-cancel', function() {
		...
		$.ajax({
			method: 'POST',
			url: site_url('prodata/pro/deleteProImage'),
			dataType: 'json',
			data:fdata,
			success: function (results) {
				console.log(results);
    ...
    if (window.File && window.FileList && window.FileReader) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            picReader.addEventListener('load', function (event) {
				...
				//console.log(fdata);
				fdata += '&pro_id='+data_id;
				$.ajax({
					method: 'POST',
					url: site_url('prodata/pro/uploadfile'),
					dataType: 'json',
					data : fdata,
					success: function (results) {
						console.log(results);
