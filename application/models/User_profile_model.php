...
class User_profile_model extends MY_Model
{
	...
	*/
	...
	{
		$search_field 	= $this->session->userdata('user_manage_search_field');
		$value 	= $this->session->userdata('user_manage_search_value');
		$value 	= trim($value);
    ...
    public function update_password($data, $user_id = NULL)
    {
        ...
        if($query->num_rows() > 0)
        {
			...
			$check_pass = FALSE;
			if($user_id == NULL){//from user edit
