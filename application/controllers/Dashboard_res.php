...
class Dashboard_res extends CRUD_Controller
{
	...
	public function index()
	{
		...
		foreach ($rows as $key => $value) {
			$shop_food_menu_images =  $shop_food_menu_images . '<div class="card col-sm-3" style="padding:10px">
          <img class="pic" src="' . base_url() . $value['encrypt_name'] . '" height="200">
        </div>';
		}
		$this->data['shop_food_menu_images'] = $shop_food_menu_images;

		$rows = $this->common_model->custom_query("select * from shop_images where shop_id=" . $this->session->userdata('shop_id') . " and fag_allow!='delete' order by image_id ASC");
		$this->data['count_image'] = count($rows);
		$allshops_images = "";
		foreach ($rows as $key => $value) {
			$allshops_images =  $allshops_images . '<div class="card col-sm-3" style="padding:10px">
          <img class="pic" src="' . base_url() . $value['encrypt_name'] . '" height="200">
        </div>';
		}
		$this->data['allshops_images'] = $allshops_images;
		$this->dashboard();
	}
