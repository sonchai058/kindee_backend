 ...
 * @date 2019-12-05
 */
class Pro_model extends MY_Model
{
	...
	{
		$pro_id = checkEncryptData($data['pro_id']);
		$this->set_where("$this->my_table.pro_id = $pro_id");
	...
	{
		$this->set_where("$this->my_table.pro_id = $id");
	...
	public function read($start_row, $per_page)
	{
		...
		if ($search_field != '' && $value != '') {
			...
			$search_method_value = '';
			if ($search_field == 'pro_name') {
				$search_method_value = "LIKE '%$value%'";
			}
			if ($search_field == 'pro_type') {
				$search_method_value = "LIKE '%$value%'";
			}
