...
/**
 * Shop_promotions_model Class
 * @date 2019-12-07
 */
class Shop_promotions_model extends MY_Model
{
	...
	public function create($post)
	{
		...
		$data = array(
				'user_update' => $post['user_update']
				,'datetime_update' => setDateToStandard($post['datetime_update'])
		);
	...
	*/
	public function read($start_row, $per_page)
	{
		...
		if ($where != '') {
			$this->db->join('users AS users_1', "$this->my_table.user_update = users_1.user_id", 'left');
		...
		$this->db->select("$this->my_table.*, users_1.user_fname AS userUpdateUserFname
				");
		$this->db->join('users AS users_1', "$this->my_table.user_update = users_1.user_id", 'left');
