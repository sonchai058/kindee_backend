...
/**
 * Nutritionist_model Class
 * @date 2019-12-05
 */
class Nutritionist_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'users_nutritionist';
	...
	{
		$user_nutri_id = checkEncryptData($data['user_nutri_id']);
		$this->set_where("$this->my_table.user_nutri_id = $user_nutri_id");
	...
	{
		$this->set_where("$this->my_table.user_nutri_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
			'date_public' => setDateToStandard($post['date_public']), 'blog_name' => $post['blog_name'], 'blog_detail' => $post['blog_detail'], 'fag_allow' => 'allow'
		);
	...
	public function read($start_row, $per_page)
	{
		...
		$where	= '';
		$order_by	= 'users_nutritionist.datetime_update DESC';
		if ($this->order_field != '') {
		...
		// $total_row = $this->db->where($this->my_table.".user_level = 'user'"."and"." ".$this->my_table.".fag_allow!='delete'")->get($this->my_table)->num_rows();
		...
		if ($where != '') {
			...
			$this->db->join('users', "$this->my_table.user_id = users.user_id", 'left');

			// $this->set_where("$this->my_table.user_level = 'user'" ."AND".$where);
	...
	public function update($post)
	{
		...
		$this->set_where("$this->my_table.blog_id = $blog_id");
