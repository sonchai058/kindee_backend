...
class Package_model extends MY_Model
{
	...
	{
		$package_id = checkEncryptData($data['package_id']);
		$this->set_where("$this->my_table.package_id = $package_id");
	...
	{
		$this->set_where("$this->my_table.package_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
			'date_public' => setDateToStandard($post['date_public']), 'package_name' => $post['package_name'], 'package_detail' => $post['package_detail'], 'fag_allow' => 'allow'
		);
	...
	public function read($start_row, $per_page)
	{
		...
		if ($search_field != '' && $value != '') {
			...
			if ($search_field == 'package_detail') {
				$search_method_value = "LIKE '%$value%'";
			}
	...
	{
		$data = array(
			'date_public' => setDateToStandard($post['date_public']), 'package_name' => $post['package_name'], 'package_detail' => $post['package_detail']
			//,'fag_allow' => $post['fag_allow']
		);

		$package_id = checkEncryptData($post['encrypt_package_id']);
		$this->set_where("$this->my_table.package_id = $package_id");
