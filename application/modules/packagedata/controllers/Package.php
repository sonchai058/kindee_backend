...
class Package extends CRUD_Controller
{
	...
	public function formValidate()
	{
		...
		$frm->set_rules('package_name', 'หัวข้อ', 'trim|required');
		$frm->set_rules('package_detail', 'รายละเอียด', 'trim|required');
	...
	{
		$post = $this->input->post(NULL, TRUE);
		$message = '<strong>ตั้งค่า package image สำเร็จ</strong>';
		$upload_error = 0;
		$upload_error_msg = '';
		$success = TRUE;
		//$encrypt_id = '';
		...
		if (isset($post['data'])) {
			...
			$success = FALSE;
			$message = '<strong>ตั้งค่า package image ล้มเหลว</strong>';
		}
		$json = json_encode(array(
			'is_successful' => $success,
			//'encrypt_id' =>  $encrypt_id,
	...
	{
		$post = $this->input->post(NULL, TRUE);
		$message = '<strong>ลบ package image สำเร็จ</strong>';
		$upload_error = 0;
		$upload_error_msg = '';
		$success = TRUE;
		//$encrypt_id = '';
		...
		if ($id != '') {
			...
			$success = FALSE;
			$message = '<strong>ลบ package image ล้มเหลว</strong>';
		}
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			if ($result == false) {
				...
				$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>' . $this->Package->error_message;
				$ok = TRUE;
			}
			$json = json_encode(array(
				'is_successful' => $ok,
